			...
			.datagrid(
					{
						...
						url : "${ctx}/employee/queryList", //数据来源
						sortName : 'employee.id', //排序的列
						sortOrder : 'desc', //倒序
						...
						columns : [ [ {
							...
							width : 2
						}, //显示复选框
								{
									...
									sortable : true,
									formatter : function(value, row, index) {
										return row.employee.pwd;
									}
								}, {
									field : 'employee.roletype',
									title : '角色',
									width : 30,
									sortable : true,
									formatter : function(value, row, index) {
										return row.employee.roletype; //该列的值是deptId，显示的是deptName
									}
								}, {
									field : 'employee.tel',
									title : '电话',
									...
									sortable : true,
									formatter : function(value, row, index) {
										return row.employee.tel; //该列的值是deptId，显示的是deptName
									}
								}] ],
						toolbar : [ {
							text : '新增',
						...
						onLoadSuccess : function() {
							$('#employeeTable').datagrid('clearSelections'); //一定要加上这一句，要不然datagrid会记住之前的选择状态，删除时会出问题
					}
					});
	$('#addRoleType').combogrid( {
		idField : 'rolename', //ID字段
...
function updaterow() {
	...
	$("#edit").dialog("open");
	$("#employeeForm").form('load', rows[0].employee);
}
...
function deleterow() {
	$.messager.confirm('提示', '确定要删除吗?', function(result) {
		if (result) {
			...
			$.post('${ctx}/employee/delete' + ps, function(data) {
...
//表格查询
function searchUser() {
	var params = $('#employeeTable').datagrid('options').queryParams; //先取得 datagrid 的查询参数
	var fields = $('#queryForm').serializeArray(); //自动序列化表单元素为JSON对象
	$.each(fields, function(i, field) {
		params[field.name] = field.value; //设置查询参数
		});
	$('#employeeTable').datagrid('reload'); //设置好查询参数 reload 一下就可以了
}
//清空查询条件
function clearForm() {
...
function addOrUpdateUser() {
	var r = $('#employeeForm').form('validate');
	if (!r) {
		return false;
	}
	$.post("${ctx}/employee/addOrUpdate", $("#employeeForm").serializeArray(),
			function(data) {
				$("#edit").dialog("close");
				$('#employeeTable').datagrid('reload');
				$.messager.alert('提示', data.mes, 'info');
    ...
    function InitRoleMenu() {
        ...
        var rows = $('#employeeTable').datagrid('getSelections');
        if (rows.length == 0) {
			$.messager.alert('提示', "请选择你要设置的员工", 'info');
			return;
		}
        var empid=rows[0].employee.id;
        $("#edit_tree").dialog("open");
        
        $("#edit-buttons_tree a:first").attr("onclick", "submitTree(" + empid + "); return false;");
        $('#tree').tree({
            url: '${ctx}/employee/getEmpOrgByEmpid?empid='+empid,
            checkbox: true,
    ...
    function submitTree(empid) {
		...
		$.post('${ctx}/employee/saveByEmpID' + s, function(data) {
	...
	<body>
		...
		<div id="edit" class="easyui-dialog" title="编辑角色信息"
			...
			<form id="employeeForm" method="post"
				...
				<br>
				账号：
				<input name="empno" style="width: 200"
					class="easyui-validatebox" required="true">
				<br>
				密码：
				<input name="pwd" style="width: 200"
					class="easyui-validatebox" required="true">
				<br>
				电话：
				<input name="tel" min="1" max="200" style="width: 200px;" required="true"></input>
				<br>
				角色：
				<input id="addRoleType" name="roletype" style="width: 200" required="true">
				<br>
		...
		<div id="edit_tree" class="easyui-dialog" title="设置员工管理的机构"
