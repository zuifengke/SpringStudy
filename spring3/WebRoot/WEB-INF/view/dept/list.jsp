  	...
  	<script type="text/javascript">
  	 jQuery(function($){
		$('#userTable').datagrid({
			...
			height: $(parent.document).find("#tabs").height() - 50 > 0 ? $(parent.document).find("#tabs").height() - 50 : 360, //高度
			fitColumns: true, //自动调整各列，用了这个属性，下面各列的宽度值就只是一个比例。
			striped: true, //奇偶行颜色不同
			collapsible:true,//可折叠
			url:"dept/queryList", //数据来源
			sortName: 'dept.id', //排序的列
			sortOrder: 'desc', //倒序
			...
			columns:[[
				...
				{field:'dept.name',title:'名字',width:20,sortable:true,
					formatter:function(value,row,index){return row.dept.name;} //需要formatter一下才能显示正确的数据
				},
				{field:'dept.code',title:'代码',width:20,sortable:true,
					formatter:function(value,row,index){return row.dept.code;}
				}
			]],
			toolbar:[{
				text:'新增',
    ...
    function updaterow(){
		...
		showWindow({
  			...
  			//而spring mvc中表单元素的名称不带对象前缀，直拉就是id，所以这里load的时候是：.form('load', rows[0].user)
  				$("#userForm").form('load', rows[0].dept);
  			}
  		});
	}
